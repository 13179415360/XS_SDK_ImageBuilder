...
ifeq ($(SDK),1)
  ...
  REVISION=$(shell $(TOPDIR)/scripts/create_xs_revision.sh)
...
# There may be packages depending on BOARD variable to determine if it should
# be visible to the build system, but in DUMP stage variable BOARD is not
# available. Try fetching it from .config if it exists and passing it to
# SCAN_MAKEOPTS in package scanning step.
ASSUMED_BOARD_FILE := tmp/.assumed_board

ifneq (,$(wildcard $(TOPDIR)/.config))
  ASSUMED_BOARD := $(shell sed -nr 's/^CONFIG_TARGET_([^_]+)=y$$/\1/gp' $(TOPDIR)/.config)
  ifneq (,$(ASSUMED_BOARD))
    PACKAGE_SCAN_MAKEOPTS := CONFIG_MTK_PLATFORM=$(shell sed -nr 's/^CONFIG_MTK_PLATFORM="([^"]+)"$$/\1/gp' $(TOPDIR)/.config)
  endif
endif

prepare-tmpinfo: FORCE
	if [ -n "$(ASSUMED_BOARD)" ]; then \
		if [ -f $(ASSUMED_BOARD_FILE) ]; then \
			if [ "$(ASSUMED_BOARD)" != "`cat $(ASSUMED_BOARD_FILE)`" ]; then \
				echo "Board changed (\"`cat $(ASSUMED_BOARD_FILE)`\" -> \"$(ASSUMED_BOARD)\"). Rescan info..."; \
				rm -rf tmp; \
			fi; \
		elif [ -f tmp/.build ]; then \
			echo "Remove existing tmp/ to recollect info..."; \
			rm -rf tmp; \
		fi; \
	fi
	@+$(MAKE) -r -s staging_dir/host/.prereq-build $(PREP_MK)
	mkdir -p tmp/info
	@echo "$(ASSUMED_BOARD)" > tmp/.assumed_board
	$(_SINGLE)$(NO_TRACE_MAKE) -j1 -r -s -f include/scan.mk SCAN_TARGET="packageinfo" SCAN_DIR="package" SCAN_NAME="package" SCAN_DEPTH=5 SCAN_EXTRA="" SCAN_MAKEOPTS="$(PACKAGE_SCAN_MAKEOPTS)"
...
defconfig: scripts/config/conf prepare-tmpinfo FORCE
	...
	$(call custom_defconfig,.config)
	[ -L .config ] && export KCONFIG_OVERWRITECONFIG=1; \
		$< $(KCONF_FLAGS) --defconfig=.config Config.in
	$(call custom_defconfig_post,.config)
...
menuconfig: scripts/config/mconf prepare-tmpinfo FORCE
	...
	$(if $(wildcard .config),$(call custom_defconfig,.config))
	[ -L .config ] && export KCONFIG_OVERWRITECONFIG=1; \
		$< Config.in
	$(call custom_defconfig_post,.config)
...
kernel_savedefconfig: export PATH:=$(dir $(DISTRO_PKG_CONFIG)):$(PATH)
endif
...
package/symlinks:
	...
	./scripts/feeds install -f -a -p xsoui
