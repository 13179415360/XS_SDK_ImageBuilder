...
define BuildKernel
  ...
  savedefconfig: $(STAMP_PREPARED) $(STAMP_CHECKED) FORCE
	rm -f $(LINUX_DIR)/.config.prev
	rm -f $(STAMP_CONFIGURED)
	$(LINUX_RECONF_CMD) > $(LINUX_DIR)/arch/$(LINUX_KARCH)/configs/$(SUBTARGET)_defconfig
	$(_SINGLE)$(KERNEL_MAKE) \
		$(if $(findstring Darwin,$(HOST_OS)), \
			HOST_LOADLIBES="-L$(STAGING_DIR_HOST)/lib -lncurses" \
			HOSTLDLIBS_mconf="-L$(STAGING_DIR_HOST)/lib -lncurses" \
		...
		YACC=$(STAGING_DIR_HOST)/bin/bison \
		$(SUBTARGET)_defconfig
	$(call LINUX_RECONF_DIFF,$(LINUX_DIR)/.config) > $(LINUX_RECONFIG_TARGET)
	rm -f $(LINUX_DIR)/arch/$(LINUX_KARCH)/configs/$(SUBTARGET)_defconfig
