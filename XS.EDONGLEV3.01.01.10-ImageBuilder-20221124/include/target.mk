...
DEFAULT_PACKAGES:=\
	...
	urngd
ifeq ($(CONFIG_IB),y)
  $(shell touch $(TOPDIR)/$(SUBTARGET)_opkg.list)
  PKG_LIST:=$(shell cat $(TOPDIR)/$(SUBTARGET)_opkg.list)
  DEFAULT_PACKAGES+=$(PKG_LIST)
endif
...
ifeq ($(DUMP),)
  # Make sure assumed board defined in prepare-tmpinfo stage matches BOARD
  ifneq (,$(wildcard $(TMP_DIR)/.assumed_board))
    _assumed_board := $(shell cat $(TMP_DIR)/.assumed_board)
    ifneq ($(BOARD),$(_assumed_board))
      $(error Assumed board $(_assumed_board) does not equal to BOARD $(BOARD). Please make sure the board defined in the base .config used for initialization is identical with the one selected via make menuconfig or defconfig.)
    endif
  endif
